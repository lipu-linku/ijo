...
COLUMN_ALIASES = {
    ...
    "Did you also take part in the 2025 word survey?": "meta_lastyear",
    ...
    "core words (117 total, minus ale) ": "w_core_all_",
    "core words (24 / 117 total, minus ale) ": "w_core_",
    "core words (23 / 117 total, minus ale) ": "w_core_",
...
START = dt.fromisoformat("2026-08-08")
...
# collapse the split core-word grids
core_all = [c for c in df.columns if
c.startswith("w_core_all_")]
for col in core_all:
    word = col.removeprefix("w_core_all_")
    target = f"w_core_{word}"
    if target in df.columns:
        df[target] = df[target].combine_first(df[col])
    else:
        df[target] = df[col]
df = df.drop(columns=core_all)
...
print("Fake word users:", len(df[(df.w_oblig_jalate > 0)]))
