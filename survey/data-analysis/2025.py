...
def get_last_year():
    series = pd.read_csv("2024.csv", index_col=0)["value"]
    return series
...
SOURCES = [
    # Main sheet
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSpsb6IgXUxoqNmkOITi3nUKzcVa3wPL5FMyDJD_2F9sst7BV6H_mgenjCoCAK3W7rWUl4fZ_RixLRr/pub?gid=154562076&single=true&output=csv",
    # No-Google account sheet
    "https://docs.google.com/spreadsheets/d/e/2PACX-1vSpsb6IgXUxoqNmkOITi3nUKzcVa3wPL5FMyDJD_2F9sst7BV6H_mgenjCoCAK3W7rWUl4fZ_RixLRr/pub?gid=678760766&single=true&output=csv",
]
...
COLUMN_ALIASES = {
    ...
    "Did you also take part in the 2024 word survey?": "meta_lastyear",
    ...
    "common, uncommon words (23 total) ": "w_oblig_",
    "obscure words (40 total) ": "w_obsc_",
    "candidate words (14 total) ": "w_obsc_",
...
START = dt.fromisoformat("2025-08-09")
...
print("Fake word users:", len(df[(df.w_oblig_anake > 0)]))

# Delete rows that erroneously report using fake words
df = df.drop(df[(df.w_oblig_anake > 0)].index)

# Output the vocab size distribution
# df.loc[:, df.columns.str.contains("w_")].sum(axis=1).sort_values().to_csv("vocab_size_2025.csv")

# Delete rows that have an untrustworthy high number of words selected
# df = df[df.loc[:, df.columns.str.contains("w_")].sum(axis=1) <= 160]

# Try deleting rows that are too recent
# df = df.drop(df[(df.meta_xp == "0-3 months")].index)

# Try deleting rows that are too old
# df = df.drop(df[(df.meta_xp == ">5 years")].index)
# df = df.drop(df[(df.meta_xp == "2-5 years")].index)
# df = df.drop(df[(df.meta_xp == "1-2 years")].index)
# df = df.drop(df[(df.meta_xp == "6-12 months")].index)
...
scores = pd.DataFrame({
    ...
    "time: 2-5 years": get_scores(df.query('meta_xp=="2-5 years"')),
    "time: 1-2 years": get_scores(df.query('meta_xp=="1-2 years"')),
    "time: 6-12 months": get_scores(df.query('meta_xp=="6-12 months"')),
    "time: 3-6 months": get_scores(df.query('meta_xp=="3-6 months"')),
    "time: 0-3 months": get_scores(df.query('meta_xp=="0-3 months"')),
...
scores.to_csv("scores-2025.csv", sep="\t")
